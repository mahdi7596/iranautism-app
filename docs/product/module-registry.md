...
| M09 | User Accounts, Authentication, and Profiles | In progress | Mobile OTP, profile, donation history, settings, possible follows/updates. Current implemented database slice starts only with the agreed `USERS` table in `docs/product/modules/iranautism-database-table-design.md`. | OTP provider, project follow feature, privacy/data retention, whether guest donation requires OTP, auth/admin schema design. |
