...
| M09 | User Accounts, Authentication, and Profiles | In progress | Mobile OTP login/register, profile, Pump mission history, donation history, settings, possible follows/updates. Current implemented database slice starts only with the agreed `USERS` table in `docs/product/modules/iranautism-database-table-design.md`. | OTP provider production settings, project follow feature, privacy/data retention, whether non-Pump guest donation requires OTP, auth/admin schema design. |
